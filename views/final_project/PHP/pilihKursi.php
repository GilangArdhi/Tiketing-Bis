...
<body>
                    ...
                    <div class="row">
                        <div class="col-3">
                            <img src="<?php echo base_url() ?>assets/Pemesanan_Tiket_Bus/steeringwheel.jpeg ?>"
                                class="img-fluid w-25 mt-2 mx-5" alt="steeringwheel">
                        </div>
                        <div class="col-9">
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                <?php echo form_open('Home/pindahPesan') ?>
                                <button type="submit" class="btn btn-success px-4" data-bs-dismiss="modal"
    ...
    <script>
        ...
        var maxvalue = <?php echo json_encode($objFilter['jmlPenumpang']) ?>;

        function highlightKursiTerpesan() {
            var kursiTerpesan = <?php echo json_encode($kursiTerpesan) ?>;
            const kursiDipesan = kursiTerpesan.map(item => item.kursiDipesan).join(', ');

            for (let i = 0; i < kursiElements.length; i++) {
                const kursiNumber = kursiElements[i].getAttribute('data-kursi-number');
                const kursiDipesanArr = kursiDipesan.split(', ');

                if (kursiDipesanArr.includes(kursiNumber)) {
                    kursiElements[i].classList.add(kursiTerpilihClass);
                }
            }
        }

        // Panggil fungsi highlightKursiTerpesan untuk menjalankannya
        highlightKursiTerpesan();
        ...
        function toggleKursiTerpilih() {
            ...
            const maxPilih = maxvalue;


            var kursiTerpesan = <?php echo json_encode($kursiTerpesan) ?>;
            console.log(kursiTerpesan);
            const kursiDipesan = kursiTerpesan.map(item => item.kursiDipesan).join(', ');
            if (kursiDipesan.includes(kursiNumber)) {
                console.log('Kursi ini sudah terpesan!');
                return; // Menghentikan fungsi jika kursi terpesan
            }
